...
This installs:
- **Core ML**: transformers, accelerate, bitsandbytes, peft
- **Tokenizers**: sentencepiece, protobuf
- **Data**: datasets, tqdm
- **Multi-Judge Evaluation**: openai, google-generativeai, anthropic, langchain
- **Cloud GPU (optional)**: modal

### Step 3: Configure API Keys (Optional - ONLY for evaluation)

**IMPORTANT**: API keys are ONLY needed for evaluation (LLM judges), NOT for inference.

Create a `.env` file (only if you want to run evaluations):
```bash
# Required for evaluation with GPT-4 judge
OPENAI_API_KEY=your-openai-key-here

# Optional - for using Gemini as judge
GEMINI_API_KEY=your-gemini-key-here

# Optional - for using Claude as judge
...
# Add a secret named "openai-secret" with:
#   OPENAI_API_KEY=your-key-here
# (Optional) Add GEMINI_API_KEY and ANTHROPIC_API_KEY for multi-judge
```

**Modal Use Cases**:
- Run evaluations without using your local GPU
- Test on different GPU types (A10G, A100, H100)
- Run long evaluations overnight without keeping your PC on
...
python experiments/basic_testing.py

# 3-component pipeline
python src/models/main_pipeline.py

# Interactive chat mode with memory
python src/models/interactive_chat.py

# Ablation testing (compare pipeline variants)
python experiments/ablation_testing.py
```

### Evaluation (Optional - Requires API Keys)

**Local evaluation - Mistral generates responses, LLM judges them:**

```bash
# Run evaluation locally with GPT-4 judge
python experiments/evaluation_part1.py --num-questions 10 --judge gpt4

# Use Gemini as judge
python experiments/evaluation_part1.py --num-questions 10 --judge gemini

# Use Claude as judge
python experiments/evaluation_part1.py --num-questions 10 --judge claude

# Compare models
python experiments/comparison.py
```

**Cloud evaluation - Run on Modal cloud GPUs:**

```bash
# Run same evaluation on Modal's A10G GPU
modal run experiments/evaluation_part1.py --num-questions 10 --judge gpt4

# Specify output file
modal run experiments/evaluation_part1.py --num-questions 50 --output results.json

# The base model runs on Modal GPU, pipelines run locally
# This saves your local GPU for other tasks
```
...
│   │   ├── judging.py                # Multi-judge evaluation (GPT-4, Gemini, Claude)
│   │   └── test_judging.py           # Binary YES/NO question judging
│   │
│   └── utils/                        # Utilities
│       ├── modal_compute.py          # Modal cloud GPU integration
│       └── modal_images.py           # Modal container images
│
├── experiments/                      # Experimental scripts
│   ├── evaluation_part1.py           # Part 1: Model ablation study
│   ├── ablation_testing.py           # Pipeline component ablation
...
│   ├── *.jsonl                       # Line-delimited results
│   └── psychosis_bench_results_*.json # Safety test results
│
├── Configuration
│   ├── requirements.txt              # Dependencies (RTX 5080 + Modal)
│   ├── .env                          # API keys (create this, not in git)
│   ├── .modalignore                  # Files to exclude from Modal upload
...
#### [experiments/basic_testing.py](experiments/basic_testing.py) - Simple Inference
...
#### [src/models/main_pipeline.py](src/models/main_pipeline.py) - 3-Component Pipeline
...
#### [experiments/evaluation_part1.py](experiments/evaluation_part1.py) - Model Ablation Study
- **Purpose**: Compare different model configurations and pipeline variants
- **Supports**: Local execution (RTX 5080) and cloud GPU execution (Modal)
- **Tests**: Base Mistral, Full pipeline, No Clinical, No Opinion, Finetuned model
- **Usage**:
  - Local: `python experiments/evaluation_part1.py --num-questions 10`
  - Cloud: `modal run experiments/evaluation_part1.py --num-questions 10`

#### [experiments/ablation_testing.py](experiments/ablation_testing.py) - Pipeline Comparison
...
#### [src/models/interactive_chat.py](src/models/interactive_chat.py) - Chat Interface
...
#### [src/evaluation/judging.py](src/evaluation/judging.py) - Multi-Judge Evaluation
- **Purpose**: Centralized multi-LLM judge for evaluation
- **Judges**: GPT-4 Turbo, Gemini 1.5 Pro, Claude 3.5 Haiku
- **Metrics**: 7 standardized MentalChat16K metrics
- **Usage**: Select judge with `--judge` flag: `gpt4`, `gemini`, or `claude`

#### [src/evaluation/test_judging.py](src/evaluation/test_judging.py) - Binary Question Judging
- **Purpose**: More objective judging using YES/NO questions
- **Method**: 5 binary questions per metric, mapped to 1-10 scores
- **Advantage**: Stricter, more reproducible grading
- **Judges**: Same three judges (GPT-4, Gemini, Claude)

#### [src/models/pipeline_pieces.py](src/models/pipeline_pieces.py) - Reusable Components
...
#### [src/utils/modal_compute.py](src/utils/modal_compute.py) - Cloud GPU Integration
- **Purpose**: Run model evaluations on Modal cloud GPUs (A10G, A100)
- **Features**: Automatic environment setup, model loading on remote GPU
- **Usage**: `modal run experiments/evaluation_part1.py --num-questions 10`
- **Cost**: Pay-per-second GPU usage (A10G: ~$1.10/hour)
